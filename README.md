...
**Confidence-aware graph augmentation for coordinated payment fraud detection.**
...
## Solution
SentinelGraph augments a transaction-level ML model with interpretable temporal relationship signals and confidence-gates their influence. When graph evidence is sparse or unreliable, the system gracefully degrades toward the ML-only score.
...
Graph-based fraud detection itself is established. SentinelGraph's contribution is the safe, interpretable, gracefully-degrading use of relationship evidence plus honest held-out evaluation.
...
## Dataset candidate
IEEE-CIS Fraud Detection. Raw competition data is not committed. Place `train_transaction.csv` and `train_identity.csv` under `data/raw/`, then run `python scripts/check_dataset.py`.
