...
def load_ieee_cis(data_dir):
    ...
    if not tx_path.exists(): raise FileNotFoundError(f"Missing {tx_path}")
    if not id_path.exists(): raise FileNotFoundError(f"Missing {id_path}")
...
def chronological_split(df, time_col="TransactionDT", train_frac=0.70, val_frac=0.15):
    ...
    n=len(ordered); a=int(n*train_frac); b=int(n*(train_frac+val_frac))
