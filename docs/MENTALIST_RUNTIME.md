...
The runtime policy therefore freezes the exact validation-derived boundary values already produced by the successful experiments:
...
1. Run the frozen v0.5 transaction + trusted-memory policy.
...
5. Apply these rules:
   ...
   - a v0.5 `ALLOW` becomes `VERIFY` only when at least two independent clue families are active and the Mentalist score is at or above the frozen Jane cutoff;
...
The two thresholds are distribution-calibrated operating boundaries, not fraud probabilities. The 6% intervention level remains a validation calibration target rather than a guaranteed future traffic share.
...
The first script reads the full-precision values from the local v0.8 and v1.0 JSON artifacts and writes `artifacts/results/mentalist_runtime_policy.json`.
...
If the fixed contract fails this reproduction gate, the held-out test remains sealed and the runtime policy must not be promoted.
